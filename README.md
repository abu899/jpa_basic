...
`.`을 찍어 객체 그래프를 탐색 하는 것

<p align="center"><img src="/imgs/18.png" width="80%"></p>
...
- 컬렉션 값 연관 경로
  - **묵시적 내부 조인** 발생
  - 탐색이 발생하지 않는다
  - 만약 FROM 절에서 명시적 조인을 통해 별칭을 얻으면 별칭을 통해서 탐색이 가능하다

> SQL 튜닝하기 어려워지기 때문에, 사실 실무에서는 묵시적인 내부조인이 발생하지 않게하는게 좋다.
...
- join fetch 명령어 사용
  - SQL 한번에 회원을 조회하며 연관된 팀도 함께 조회하고 싶다
...
- 일반 join과의 차이점
  - 일반 join은 연관된 엔티티를 함께 조회하지 않는다
    - 단지 SELECT 절에 지정된 엔티티만 조회할 뿐
  - 따라서 영속성 컨텍스트에 없는 데이터를 가져올 때 쿼리가 추가 실행된다.
- fetch join을 사용한다는 건 사실상 연관된 엔티티를 `즉시 로딩`하는 개념

### Fetch Join의 특징과 한계

- 패치 조인 대상에는 alias를 설정할 수 없다
  - 하이버네이트는 가능하지만, 가급적 사용하지 않는 것이 관례이다 
  - 패치 조인이라는 건 기본적으로 연관된 엔티티를 모두 가져오는 것
- 둘 이상의 컬렉션은 패치 조인할 수 없다
- 컬렉션 패치 조인하면 페이징 API를 사용할 수 없다
  - 일대일, 다대일 같은 단일 값 연관 필드들은 상관없이 페이징 가능
  - 하지만 이외의 컬렉션 패치 조인의 경우 데이터가 늘어남에 따라 페이징이 위험하다

### Fetch Join 정리

- N+1과 관련된 문제 대부분은 패치 조인으로 해결 가능하다
- 패치 조인은 객체 그래프를 유지할 때 사용하는 것이 효과적이다
  - `SELECT m from Member m join fetch m.team` 에서 m.team 같은
- 여러 테이블을 조인해서 엔티티가 가진 모양이 아닌 다른 결과를 내야될 때는 일반 조인!
  - 필요한 데이터들만 조회해서 DTO로 반환하는 것이 효과적이다

### 엔티티 직접 사용

`select count(m) from Member m`
- SQL에서는 엔티티의 기본 키 값을 사용하게 된다
  - 즉 SQL은 `select count(m.id) from Member m` 이 실행된다
- 엔티티와 연관된 다른 엔티티에서는 어떻게 될까
  - `select m from Member m where m.team = :team`
  - 이 경우는 연관된 엔티티의 외래키 값을 사용하게 된다
  - 즉 SQL은 `select m.* from Member m where m.team_id = ?` 로 동작한다

### Named Query

- 쿼리에 이름을 부여하는 것으로 미리 정의해서 사용하는 JPQL
- 정적 쿼리에 사용 가능하다
...
- 일반적으로 SQL의 update, delete 문이라고 생각하면 된다
- 만약 재고가 10개 미만인 **모든 상품**의 가격을 10%로 상승 시킨다
  - JPA의 변경감지(dirty-checking)을 이용하려면
    - 재고가 10개 미만인 상품 리스트 조회
    - 상품 엔티티 가격 상승 적용
    - 트랜잭션 커밋 시점에 더티 체킹 동작
  - 변경된 데이터가 100건이라면 100개의 update sql이 실행된다
- 즉 쿼리 한번으로 여러 테이블 로우를 변경하는 것이 벌크 연산!
- `Hibernates`의 경우 `insert into ..select`도 지원한다

### 벌크 연산의 주의점

- 벌크 연산은 영속성 컨텍스트를 무시하고 데이터베이스에 직접 쿼리를 날린다
  - 즉 벌크 연산을 하고나서도 영속성 컨텍스트는 변경한 점을 알 수 없다
- 따라서 다음 방법으로 해결하는 게 좋다
  - 벌크 연산을 우선적으로 실행한다
  - 아니면 벌크 연산을 수행하고 나서 반드시 영속성 컨텍스트를 초기화 해준다